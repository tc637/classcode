...
if __name__=="__main__":
    ...
    for the_lapse_rate in dT_dz:
        ...
        bright_dict=OrderedDict() # brightness temperature dictionary
        for wavel,k_lambda in wavel_k_tup:
            ...
            bright_value = planckInvert(wavel*1.e-6,rad_value) # in K
            rad_dict[wavel]=rad_value
            bright_dict[wavel]=bright_value
        rad_profs.append(rad_dict)
        bright_profs.append(bright_dict)

    plt.close('all')
    fig1,axis1=plt.subplots(1,1)
    fig2,axis2=plt.subplots(1,1) # brightness temperatures
    fig3,axis3=plt.subplots(1,1) # brightness differences in each atmosphere
    ...
    bright_diffs = []
    # plot brightness temperatures and differences
    for index,the_profile in enumerate(bright_profs):
        wavelengths=the_profile.keys()
        brightnesses=np.array(the_profile.values())
        # calculate brightness range for each dT_dz
        bright_diff = abs(max(brightnesses) - min(brightnesses)) 
        bright_diffs.append(bright_diff)
        axis2.plot(wavelengths,brightnesses,label=str(dT_dz[index]*1.e3))
    ...
    axis2.set_title('brightness temperatures at top of atmosphere for 7 values of dT/dz (K/km)')
    axis2.set_ylabel('brightness temperature (K)')
    axis2.set_xlabel('wavelength (microns)')
    axis2.legend(loc='best')
    fig2.savefig('brightnesses.png')

    axis3.plot(dT_dz*1.e3,bright_diffs)
    axis3.set_title('brightness temperature range for each atmospheric lapse rate')
    axis3.set_ylabel('brightness temperature range (K)')
    axis3.set_xlabel('lapse rate (K/km)')
    fig3.savefig('differences.png')
    plt.show()
